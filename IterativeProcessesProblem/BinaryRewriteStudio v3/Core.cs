...
namespace BinaryRewrite
{
    ...
    // -----------------------------------------------------------------------------------
    /// <summary>Per-seed outcome, collected only when a survey is small enough to list.</summary>
    public sealed class SeedOutcome
    {
        ...
        public bool NuCapped;   // stopped because nu exceeded the cap (presumed non-halting)

        public string Outcome => Halted
            ? $"halt @ step {HaltStep}  (s={HaltCounter})"
            : NuCapped ? "non-halting (ν cap hit)" : "non-halting (step cap)";
    ...
    public sealed class SurveyResult
    {
        ...
        public List<SeedOutcome> Seeds = new List<SeedOutcome>(); // populated only if collecting
        public bool SeedsListed;                                  // true if Seeds is complete
        public long Halting, NonHalting;
        public long NuCapped;                                     // seeds that hit the nu cap (or resource limit)
    ...
    public static class SeedSurvey
    {
        ...
        /// <param name="maxNu">cap on nu (long.MaxValue = unlimited); a seed exceeding it is presumed non-halting</param>
        /// <param name="maxCores">degree of parallelism (1..Environment.ProcessorCount)</param>
        /// <param name="collectCap">if total seeds &lt;= this, every seed's outcome is recorded for listing</param>
        /// <param name="engine">classifier engine: GapMemory (default workhorse) or Bit (ground truth, small only)</param>
        public static SurveyResult Run(int lenFrom, int lenTo, int maxSteps, long maxNu,
                                       int maxCores, long collectCap, EngineKind engine,
                                       CancellationToken ct, IProgress<double> progress = null)
        {
            ...
            long done = 0;
            bool collect = totalWork <= collectCap;
            res.SeedsListed = collect;
            ...
            for (int len = lenFrom; len <= lenTo; len++)
            {
                ...
                Parallel.For(0L, count,
                    ...
                    (idx, state, acc) =>
                    {
                        ...
                        if (engine == EngineKind.Bit) ClassifyBit(seed, maxSteps, maxNu, collect, acc);
                        else Classify(seed, maxSteps, maxNu, collect, acc);
            ...
            if (collect) res.Seeds.Sort((a, b) =>
            {
                int c = a.Seed.Length.CompareTo(b.Seed.Length);
        ...
        private static void Classify(string seed, int maxSteps, long maxNu, bool collect, SurveyResult acc)
        {
            acc.TotalSeeds++;
            var outcome = collect ? new SeedOutcome { Seed = seed } : null;
            var ge = GapEngine.FromSeed(seed);
            ...
            for (int n = 0; n <= maxSteps; n++)
            {
                if (ge.Nu > maxNu)
                {
                    ...
                    if (outcome != null) outcome.NuCapped = true;
                    goto post;
                }
                bool ok;
                long s;
                try { ok = ge.Step(out s, out _); }
                catch (Exception)   // out of memory / overflow on a runaway seed
                {
                    acc.NonHalting++; acc.NuCapped++;
                    if (outcome != null) outcome.NuCapped = true;
                    goto post;
        ...
        // Bit-engine classification: identical bookkeeping, slower, materialises strings.
        private static void ClassifyBit(string seed, int maxSteps, long maxNu, bool collect, SurveyResult acc)
        {
            acc.TotalSeeds++;
            var outcome = collect ? new SeedOutcome { Seed = seed } : null;
        ...
        private static void Merge(SurveyResult a, SurveyResult b)
        {
            ...
            a.Halting += b.Halting; a.NonHalting += b.NonHalting; a.NuCapped += b.NuCapped;
