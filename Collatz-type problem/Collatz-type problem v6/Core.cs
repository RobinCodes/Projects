...
namespace BinaryRewrite
{
    ...
    public sealed class StepInfo
    {
        ...
        public long R1 = -1;          // r1(L_n): length of the first maximal 1-run (Prop 16)
    ...
    public sealed class GapEngine
    {
        ...
        /// <summary>r1: length of the first maximal 1-run = 1 + (leading run of zero-gaps).</summary>
        public long R1()
        {
            long r = 1;
            for (int i = 0; i < Gaps.Length; i++) { if (Gaps[i] == 0) r++; else break; }
            return r;
        }
    ...
    //  session root so it can be wiped in a single call when the program closes.
    //
    //  Guarantee: spill files NEVER survive a session. Three layers ensure this:
    //    1. each DiskGapEngine deletes its own directory on Dispose,
    //    2. normal shutdown calls Cleanup() to wipe the whole session root,
    //    3. on every startup, SweepStaleSessions() deletes any session left behind by a
    //       previous run that was killed/crashed before it could clean up.
    //  A per-session lock file lets the sweep tell a crashed session (lock free) apart
    //  from another instance still running concurrently (lock held), so we never delete
    //  a live sibling's data.
    // -----------------------------------------------------------------------------------
    public static class DiskWorkspace
    {
        public static readonly string BaseDir =
            Path.Combine(Path.GetTempPath(), "BinaryRewriteStudio");

        public static readonly string Root =
            Path.Combine(BaseDir, "session-" + Guid.NewGuid().ToString("N"));

        private static readonly string LockPath = Root + ".lock";
        private static FileStream _lock;     // held open for the whole process lifetime
        private static readonly object _gate = new object();
        private static bool _rootReady;
        private static int _ctr;

        /// <summary>Create the session root (once) and take its lock. Thread-safe.</summary>
        private static void EnsureRoot()
        {
            ...
            lock (_gate)
            {
                ...
                try
                {
                    _lock = new FileStream(LockPath, FileMode.Create, FileAccess.ReadWrite,
                                           FileShare.None, 8, FileOptions.DeleteOnClose);
                }
                catch { /* lock is best-effort; cleanup still runs on close */ }
                _rootReady = true;
            }
        }
        ...
        /// <summary>Delete every file this session wrote to disk, and release the lock. Safe to call repeatedly.</summary>
        public static void Cleanup()
        {
            lock (_gate)
            {
                try { _lock?.Dispose(); } catch { } // FileOptions.DeleteOnClose removes the .lock
                _lock = null;
        ...
        /// <summary>
        /// Delete scratch directories left behind by previous runs (crash/kill). A session
        /// whose lock file is free (or absent) is dead and gets removed; one whose lock is
        /// still held by a live process is skipped. Call once at startup.
        /// </summary>
        public static void SweepStaleSessions()
        {
            try
            {
                if (!Directory.Exists(BaseDir)) return;
                foreach (var dir in Directory.EnumerateDirectories(BaseDir, "session-*"))
                {
                    if (string.Equals(Path.GetFullPath(dir), Path.GetFullPath(Root), StringComparison.OrdinalIgnoreCase))
                        continue; // never our own
                    string lockFile = dir + ".lock";
                    bool dead = true;
                    if (File.Exists(lockFile))
                    {
                        try
                        {
                            // If we can open it exclusively, no live process holds it -> dead.
                            using var fs = new FileStream(lockFile, FileMode.Open, FileAccess.ReadWrite, FileShare.None);
                        }
                        catch { dead = false; } // still locked by a running instance
                    }
                    if (dead)
                    {
                ...
                // also drop orphaned *.lock files whose dir is already gone
                foreach (var lf in Directory.EnumerateFiles(BaseDir, "session-*.lock"))
                {
                    string dir = lf.Substring(0, lf.Length - ".lock".Length);
                    if (Directory.Exists(dir)) continue;
                    try
                    {
                        using (var fs = new FileStream(lf, FileMode.Open, FileAccess.ReadWrite, FileShare.None)) { }
                        File.Delete(lf);
                    }
                    catch { }
                }
    ...
    public sealed class DiskGapEngine : IDisposable
    {
        ...
        /// <summary>r1: 1 + (leading run of zero-gaps); reads only up to the first nonzero gap.</summary>
        public long ComputeR1()
        {
            long r = 1;
            using (var br = OpenRead())
            {
                ...
                while (!done)
                {
                    int got = ReadInts(br, buf);
                    if (got == 0) break;
                    for (int i = 0; i < got; i++) { if (buf[i] == 0) r++; else { done = true; break; } }
                }
            }
            return r;
        }
    ...
    public sealed class TrajectoryRunner
    {
        ...
        private static long R1Of(string L)
        {
            int i = 0; while (i < L.Length && L[i] == '0') i++;   // skip inert leading 0's
            long r = 0; while (i < L.Length && L[i] == '1') { r++; i++; }
            return r;
        }
    }
