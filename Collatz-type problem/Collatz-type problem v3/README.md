...
`n, sₙ, parity(ν), ν(Lₙ), ω(Lₙ), |Lₙ|, V(Lₙ)` (base-10, shown exactly when it fits and as a
digit-count estimate when it doesn't). Strings are shown for small `n`, and a **Decompose**
button breaks an even-parity counter into its contributing-gap multiset plus size surplus.
In **Auto** mode the engine starts in memory and **automatically spills to disk** when the
in-memory vector hits its cap, without losing progress; the status line reports the spill
step.
...
across the cores you pick; engine choice (including the auto-spill mode) is exposed here too.
...
**Seed survey** — enumerate every seed (beginning with 1) over a length range, in parallel
across a chosen number of cores (the picker shows how many you have), and report the
Table 2 / Table 3 statistics plus the number of **ν-capped** seeds (those that hit the ν
cap or the in-memory limit). The classifier engine can be `Gap (in-memory)` or
`Bit (small, exact)`. **ν** can be left **unlimited**, or capped to stop a runaway
non-halting seed from eating memory.

For surveys of ≤ 100,000 seeds, every seed is listed at the bottom with its outcome and the
list fills the remaining space; a **filter** dropdown (`All / Halting / Non-halting /
ν-capped`) narrows the view, and **clicking any seed** opens a popup with its full
trajectory — a plot of whatever metric you choose, the written `sₙ` trajectory, and the
per-step table. A toggleable **master graph** overlays the trajectories of all listed seeds
(sampled down to 500), colour-coded green for halting and red for non-halting, so the global
shape of the orbit family is visible at a glance.

**Conjugate T** — the integer Collatz-type conjugate: `N₀ = V(L₀)`, `N_{n+1} = T(Nₙ)`.
Each `Nₙ` is cross-checked against the bit-string value `V(Lₙ)` wherever the chosen engine
can still materialise it.

Every long-running action has its own progress bar and a Cancel button.

## Closing the program

Closing the window **interrupts any running computation**, switches to a "cleaning up
temporary disk data" screen, and deletes everything the disk engines wrote under
`%TEMP%/BinaryRewriteStudio/session-<guid>/` before the program exits. Nothing the disk
engines produced survives a session.
